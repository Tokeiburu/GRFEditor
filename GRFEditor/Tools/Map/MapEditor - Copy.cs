	...
	public class MapEditor {
		...
		private Rsw _configRswFile(byte[] rswData, string mapName, Gat gat, Gnd gnd) {
			...
			if (GrfEditorConfiguration.FlattenGround) {
				...
				rsw.RebuildQuadTree(gat.Header.Width / 2, gat.Header.Height / 2);
		...
		public class GatData {
			public GetHeaderStruct Header;
			public GatCellStruct[] Cells;
		}

		private static T[] FromByteArray<T>(byte[] source, int offset) where T : struct {
			T[] destination = new T[(source.Length - offset) / Marshal.SizeOf(typeof(T))];
			GCHandle handle = GCHandle.Alloc(destination, GCHandleType.Pinned);
			try {
				IntPtr pointer = handle.AddrOfPinnedObject();
				Marshal.Copy(source, offset, pointer, source.Length - offset);
				return destination;
			}
			finally {
		...
		private Gat _configGatFile(byte[] gatData, float waterLevel) {
			...
			GatData gatRaw = new GatData();
			gatRaw.Header.Width = BitConverter.ToInt32(gatData, 6);
			gatRaw.Header.Height = BitConverter.ToInt32(gatData, 10);
			gatRaw.Cells = FromByteArray<GatCellStruct>(gatData, 14);
