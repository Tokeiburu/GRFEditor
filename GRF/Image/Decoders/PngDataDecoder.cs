...
namespace GRF.Image.Decoders
{
	...
	internal static class PngDataDecoder
    {
        public static void GetBytesAndSamplesPerPixel(IhdrData header, out byte bytesPerPixel, out byte samplesPerPixel)
        {
            var bitDepthCorrected = (header.BitDepth + 7) / 8;

            samplesPerPixel = SamplesPerPixel(header);
	        bytesPerPixel = (byte)(samplesPerPixel * bitDepthCorrected);
        }

        public static byte[] Decode(byte[] decompressedData, IhdrData header, byte bytesPerPixel, byte samplesPerPixel)
        {
                        ...
                        for (var pass = 0; pass < 7; pass++)
                        {
                            ...
                            for (var scanlineIndex = 0; scanlineIndex < numberOfScanlines; scanlineIndex++)
                            {
                                ...
                                for (var j = 0; j < numberOfPixelsPerScanline; j++) {
	                                int x, y;
                                    Adam7.GetPixelIndexForScanlineInPass(header, pass, scanlineIndex, j, out x, out y);
        ...
        private static byte SamplesPerPixel(IhdrData header)
        {
        ...
        private static int BytesPerScanline(IhdrData header, byte samplesPerPixel)
        {
